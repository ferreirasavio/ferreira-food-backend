...
class StoreController {
    ...
    async create(request: Request, response: Response){
        ...
        const documentExists = await trx("store").where("document", document).first()
        ...
        if (nameExists) {
          ...
          await trx("store").insert(request.body)
          await trx.commit();

          return response.json(`Store with created with success!`);
    }
